...
typedef struct {
    ...
    volatile uint32_t CNTL;  // 0x08
    volatile uint32_t CNTH;  // 0x0C
    volatile uint32_t CMPL;  // 0x10
    volatile uint32_t CMPH;  // 0x14
} Systick;
...
void InitSystick()
{
    ...
    systick->CMPL = 1440000;
    systick->CMPH = 0;

    systick->CTLR = (1 << 5) | (1 << 3) | (1 << 2) | (1 << 1);
}
...
void InitInterrupts()
{
    __asm__ volatile ("csrw mtvec, %0" :: "r"((uintptr_t)SysTick_Handler));
    *PFIC_IENR1 = (1u << 12);
}
